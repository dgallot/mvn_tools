...
def hightlights( str, regexes ) :
    result=str
    init()
    if "red" in regexes :        
        result=re.sub('(' + regexes["red"] + ')', Fore.RED + r'\1' + Fore.RESET, str )
    if "green" in regexes :
        result=re.sub('(' + regexes["green"] + ')', Fore.GREEN + r'\1' + Fore.RESET, str )
    return result
...
def get_ssh_keys():
    ...
    try:
        ...
        try:
            return [paramiko.RSAKey.from_private_key_file(filename=SSH_KEY, password="")]
        except paramiko.PasswordRequiredException:
            try :
                passwd = getpass.getpass("Enter passphrase for %s: " % SSH_KEY)
            except paramiko.SSHException:
                try:
                    return [paramiko.RSAKey.from_private_key_file(filename=SSH_KEY, password=passwd)]
                except paramiko.SSHException:
...
class Ssh :
    ...
    def __init__(self, hostname, instance = None, port=22, username=None, keyfile='~/.ssh/id_rsa', keyfile_password=None):
        ...
        if keyfile_password is None :
            try:
                key = paramiko.RSAKey.from_private_key_file(os.path.expanduser(keyfile), password="")
            except paramiko.PasswordRequiredException:            
